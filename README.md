...
| `scan_assets.py` | **工程体检核心库**：字符串启发式提取第三方插件 / 字体 / 素材路径 / 表达式 / AE 内置效果，给出降级与协作风险提示 | 需要 |
...
- 顶部「工程体检」标签页：**不依赖工程软件**，启发式扫描第三方插件 / 字体 / 素材路径 / 表达式 / AE 内置效果，并给出「降级 + 协作交付」风险提示（详见下方「工程体检」一节）
...
降级只改「版本标记」，但**工程能不能在旧版里正常打开、协作时丢不丢东西**，取决于工程里那些
"版本相关"的依赖。工程体检就是在不启动 After Effects / Premiere 的前提下，从工程文件里
**启发式提取这些依赖**，提前告诉你风险在哪。

网页版顶部「工程体检」标签页可独立使用，也支持命令行 `python scan_assets.py <文件>`。

### 它能查到什么

| 类别 | 怎么提取 | 风险提示 |
|---|---|---|
| **第三方插件** | 字符串匹配 Trapcode / Red Giant / Particular / Boris / Sapphire / Video Copilot 等 37 个厂商关键词 | 目标机器需装**同版本**插件，否则效果被禁用或丢失 |
| **AE 内置效果** | 正则匹配 `ADBE ××` 效果名（matchName） | 仅作信息展示，内置效果一般随版本保留 |
| **字体** | 关键词 + PR 工程的 `<Font>` 标签 | 目标机器缺字体时会被替换、版面走样 |
| **素材路径** | 正则提取磁盘绝对路径 / 相对路径（`.mov`/`.png`/`.wav`/…） | 降级只改标记、素材不动，确认目标机器路径仍可访问（或已重新链接） |
| **表达式** | 匹配 `wiggle(` / `loopOut(` / `thisComp.layer(` 等特征 | 旧版一般保留表达式，但**新版本才有的函数/方法会报错** |

### 提取原理（为什么是"启发式"）

- **AE 二进制（`.aep` / `.ffx`）**：效果名 / 字体名 / 素材路径多为 **UTF-16LE 宽字符**（字符后跟 `0x00`），
  单字节扫描会漏，所以同时做 ASCII + 宽字符两套提取；XML 工程（`.aepx`）是纯文本直接搜。
- **PR 工程（`.prproj`）**：CS6 之后是 gzip 压缩的 XML，解压后是纯文本，直接搜 `<Font>` / `<MediaPath>` / `<FilePath>` 等标签最准。
- 正则 `/[A-Za-z]:\\…\.ext` 与 `/…/…/….ext` 覆盖 Windows 绝对路径与类 Unix 相对路径。

### 缺失素材怎么核查

浏览器内只能读到工程文件本身，访问不到它引用的素材磁盘文件，所以"缺失"不能靠浏览器自动判断。两个办法：

- **选整个文件夹**：在体检页点「选择文件夹…」选存在工程的目录（或其素材目录），工具收集目录内所有文件名，
  按文件名与工程里引用的素材路径做匹配，凡是目录里**找不到同名文件**的，标记为「疑似缺失素材」。
- **只选单个工程**：则只列出素材路径，需你自行在目标机器核对这些路径是否仍可访问（或已重新链接）。
...
> 专有编码、加密、被压缩进自定义 chunk 的内容可能漏检；也有可能误命中普通文本里的关键词。
> 它给的是**风险清单**，不是保证书。最终结论仍以目标版本软件**实机打开**为准。

命令行用法：

```bash
python scan_assets.py "D:\proj\demo.aep"        # AE / FFX / AEPX
python scan_assets.py "D:\proj\demo.prproj"      # Premiere
python scan_assets.py "D:\proj" --recursive      # 整目录
```
